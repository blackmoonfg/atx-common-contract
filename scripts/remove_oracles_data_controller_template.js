...
module.exports = async callback => {
	...
	const removeOracles = async (dataController, oracle) => {
		const methods = [
		...
		if ((await dataController.removeOracles.call(params.signatures, params.oracles, { from: systemOwner, })).toNumber() === 1) {
			await dataController.removeOracles(params.signatures, params.oracles, { from: systemOwner, })
		}
	}

	for (var oracle of oracles) {
		await removeOracles(assetDataController, oracle)
	}
